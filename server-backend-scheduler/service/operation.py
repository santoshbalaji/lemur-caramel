...
class OperationService(BaseService):
    def update_operation_status(self, operation_id: int, operation_status: str) -> str:
        try:
            logging.info("------------- update operation status executed -----------")
            status, operation = self._validate_operation_id(operation_id=operation_id)
            if not status:
                logging.error(Error.ERROR_OPERATION_NOT_EXISTS.format(operation_id))
    ...
    def get_next_operation(self) -> str:
        try:
            logging.info("------------- get next operation executed -----------")
            ...
            if not running_operations or len(running_operations) == 0:
                ...
                if operations and len(operations) != 0:
                    ...
                    operations[0]['user_id'] = users[0]['user_id']
            ...
            return Message.OPERATION_NEXT_JOB_DONE
        except Exception as e:
            logging.error(Error.ERROR_OPERATION_GET_NEXT_JOB.format(str(e)))
            return Error.ERROR_OPERATION_GET_NEXT_JOB.format(str(e))

    def get_results(self, message: str) -> str:
        try:
            logging.info("------------- get results executed -----------")
            ...
            if iot_ops and 'operation_id' in iot_ops and 'status' in iot_ops:
                ...
                if operations and len(operations) != 0:
                    ...
                    self.update_operation_status(operation_id=operations[0]['idx'], operation_status=iot_ops['status'])
            return Message.OPERATION_RESULT
        except Exception as e:
            logging.error(Error.ERROR_OPERATION_GET_RESULT.format(str(e)))
